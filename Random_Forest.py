...
def predict_sex(name):
    ...
    # Map and handle NaN values by filling them with a default value (e.g., 0 for unknown)
    sex_code = sex.map(sex_dict).fillna(0).astype(int)  # Default to 0 (unknown) if no match
    return sex_code
...
y_pred, clf = train(X_train, y_train, X_test)
